...
struct Cons1DS
{
    ...
    Cons1DS(double const &dInput,
            ...
            std::vector<double> const &sInput)
        :
        d (dInput),
        E (EInput),
        Mx(MxInput),
                         ...
                         double const &Gamma)
{
    ...
    #if (NSCALARS > 0)
        Prim1D.r.resize(pU.s.size());
        for (size_t n=0; n<NSCALARS; n++) Prim1D.r[n] = pU.s[n]*di;
    #endif
                         ...
                         double const &Gamma)
{
    ...
    #if (NSCALARS > 0)
        Cons1D.s.resize(pW.r.size());
        for (size_t n=0; n<NSCALARS; n++) Cons1D.s[n] = pW.r[n]*pW.d;
    #endif
                  ...
                  double const &gamma)
{
    ...
    std::cout << "Fluxes, copy/pastable: ("
              ...
              << fluxes.Mz << ", "
              << fluxes.E  << ", ";
    for (size_t i = 0; i < fluxes.s.size(); i++)
    {
    std::cout << fluxes.s[i]  << ", ";
    }
    std::cout << "0.0"     << ", "
